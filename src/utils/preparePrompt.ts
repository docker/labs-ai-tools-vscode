...
export const prepareProjectPrompt = (projectRoot: vscode.WorkspaceFolder, promptType: string) => {

    const platform = process.platform;

    const promptImage = vscode.workspace.getConfiguration('docker.make-runbook').get('prompt-image') as string;

    const promptImageArgs = ['run', '--rm', "-v", "/var/run/docker.sock:/var/run/docker.sock", "--mount", "type=volume,source=docker-prompts,target=/prompts", promptImage, projectRoot.uri.fsPath, vscode.env.machineId, platform, promptType];
